...
- 预览缩放范围为 50%-200%，100% 保持当前“适合可用宽度且不超过 A4 基准宽度”的尺寸。
  顶部提供减小、比例预设、增大与恢复 100%；预览区域内 Ctrl/Cmd+滚轮和触摸板捏合修改实际
  容器尺寸，不使用脱离布局的 `transform: scale()`，不触发 Typst 重编译。超过视口后必须可水平和
  垂直滚动，普通滚轮不被拦截。字体授权、首次编译和无产物错误状态下禁用全部缩放入口；已有
  产物在后台重编译时仍可继续缩放当前预览。像素级触摸板手势使用适中的连续缩放灵敏度，传统
  行级与页级滚轮维持较低跳幅。
